...
@SuppressWarnings("null") // TODO: create defaultNonNull annotation to package-info.java
class JavaGenericExample {
  ...
  public <R extends Sort> BoolExpr injAxiom(Context ctx, FuncDecl<R> f, int i) {
    ...
    List<Expr<Sort>> xs =
        IntStream.range(0, sz).mapToObj(j -> ctx.mkBound(j, types[j])).collect(Collectors.toList());
    Symbol[] names = IntStream.range(0, sz).mapToObj(j -> ctx.mkSymbol(String.format("x_%d", j)))
        .toArray(Symbol[]::new);
  ...
  public <R extends Sort> BoolExpr injAxiomAbs(Context ctx, FuncDecl<R> f, int i) {
    ...
    /* allocate temporary arrays */
    List<Expr<Sort>> xs =
        IntStream.range(0, sz).mapToObj(j -> ctx.mkConst(String.format("x_%d", j), domain[j]))
  ...
  private <R extends Sort> BoolExpr commAxiom(Context ctx, FuncDecl<R> f) throws Exception {
    ...
    String bench = String.format("(assert (forall (x %s) (y %s) (= (%s x y) (%s y x))))",
        t.getName(), t.getName(), f.getName(), f.getName());
  ...
  void arrayExample1(Context ctx) throws TestFailedException {
    ...
    System.out.printf("Interpretation of MyArray:%n%s%n",
    ...
    System.out.printf("Interpretation of MyArray as Term:%n%s%n",
        s.getModel().getFuncInterp(aex.getFuncDecl()));
  }
  ...
  public void arrayExample2(Context ctx) throws TestFailedException {
    ...
    /*
     * prove store(a1, i1, v1) = store(a2, i2, v2) implies (i1 = i3 or i2 = i3 or select(a1, i3) =
     * select(a2, i3))
     */
  ...
  public void arrayExample3(Context ctx) throws TestFailedException {
    ...
    for (int n = 2; n <= 5; n++) {
      ...
      List<Expr<ArraySort<BoolSort, BoolSort>>> a =
          IntStream.range(0, n).mapToObj(i -> ctx.mkConst(String.format("array_%d", i), array_type))
              .collect(Collectors.toList());
  ...
  void sudokuExample(Context ctx) throws TestFailedException {
    ...
    List<List<Expr<IntSort>>> X = IntStream.range(0, 9)
        .mapToObj(i -> IntStream.range(0, 9)
            .mapToObj(j -> ctx.mkConst(ctx.mkSymbol(String.format("x_%d_%d", i + 1, j + 1)),
                ctx.getIntSort()))
            .collect(Collectors.toList()))
        .collect(Collectors.toList());

    // each cell contains a value in {1, ..., 9}
    List<List<BoolExpr>> cells_c = X.stream()
        .map(r -> r.stream()
            .map(c -> ctx.mkAnd(ctx.mkLe(ctx.mkInt(1), c), ctx.mkLe(c, ctx.mkInt(9))))
            .collect(Collectors.toList()))
        .collect(Collectors.toList());
    ...
    BoolExpr instance_c = ctx.mkAnd(IntStream.range(0, 9).boxed()
        .flatMap(i -> IntStream.range(0, 9).filter(j -> instance[i][j] != 0)
            .mapToObj(j -> ctx.mkEq(X.get(i).get(j), ctx.mkInt(instance[i][j]))))
        .toArray(Expr[]::new));
    ...
    if (s.check() == Status.SATISFIABLE) {
      Model m = s.getModel();
      List<List<Expr<IntSort>>> R =
          X.stream().map(r -> r.stream().map(c -> m.eval(c, false)).collect(Collectors.toList()))
              .collect(Collectors.toList());
      System.out.println("Sudoku solution:");
      R.forEach(r -> System.out
          .println(r.stream().map(Objects::toString).collect(Collectors.joining(" "))));
    } else {
  ...
  void quantifierExample1(Context ctx) {
    ...
    Symbol[] names = IntStream.range(0, 3).mapToObj(j -> ctx.mkSymbol(String.format("x_%d", j)))
        .toArray(Symbol[]::new);
    List<Expr<IntSort>> xs = IntStream.range(0, 3).mapToObj(j -> ctx.mkConst(names[j], types[j]))
        .collect(Collectors.toList());
    List<Expr<IntSort>> vars = IntStream.range(0, 3).mapToObj(j -> ctx.mkBound(2 - j, types[j]))
        .collect(Collectors.toList());
  ...
  public void quantifierExample3(Context ctx) throws TestFailedException {
    ...
    /*
     * If quantified formulas are asserted in a logical context, then the model produced by Z3
     * should be viewed as a potential model.
     */
  ...
  void basicTests(Context ctx) throws TestFailedException {
    ...
    System.out.printf("%s -> %s%n", mixedDivision2, mixedDivision2.simplify()); // (/ 1.0 (to_real
                                                                                // 2)) -> 1/2
    ...
    System.out.printf("%s -> %s%n", eq1, eq1.simplify()); // (= (/ 1.0 2.0) (to_real (div 1 2))) ->
                                                          // false

    Expr<BoolSort> eq2 = ctx.mkEq(realDivision, mixedDivision2);
    System.out.printf("%s -> %s%n", eq2, eq2.simplify()); // (= (/ 1.0 2.0) (/ 1.0 (to_real 2))) ->
                                                          // true
  }
  ...
  public void pushPopExample1(Context ctx) throws TestFailedException {
    ...
    /*
     * backtrack: the constraint x <= 3 will be removed, since it was asserted after the last
     * ctx.Push.
     */
  ...
  public <T extends Sort> void enumExampleTyped(Context ctx) throws TestFailedException {
    ...
    // helper function for consistent typing:
    ...
    prove(ctx,
        ctx.mkOr(ctx.mkEq(fruity, apple), ctx.mkEq(fruity, banana), ctx.mkEq(fruity, orange)),
        false);
  }

  // while you can do this untyped, it's safer to have a helper function -- this will prevent you
  // from
  ...
  public void listExample(Context ctx) throws TestFailedException {
    ...
    prove(ctx,
        ctx.mkOr(ctx.mkApp(int_list.getIsNilDecl(), u), ctx.mkApp(int_list.getIsConsDecl(), u)),
        false);
  ...
  public <Tree> void treeExample(Context ctx) throws TestFailedException {
    ...
    BoolExpr fml1 =
        ctx.mkEq(u, ctx.mkApp(cons_decl, ctx.mkApp(car_decl, u), ctx.mkApp(cdr_decl, u)));
  ...
  public void evalExample2(Context ctx) {
    ...
    if (Status.SATISFIABLE == solver.check()) {
      ...
      System.out.printf("evaluating second(tup2) %s%n",
  ...
  public void floatingPointExample1(Context ctx) throws TestFailedException {
    ...
    /*
     * nothing is equal to NaN according to floating-point equality, so NaN == k should be
     * unsatisfiable.
     */
